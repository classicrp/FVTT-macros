...
if (action.tag === `btstop`) {
	...
	const actBuffs = await actor._itemTypes.buff.filter(b => b.system.tag.includes('bladethirst') && b.system.active);
...
//	only Bard with Archetype ?? can use
if (typeof mCl.bard !== "undefined") {
	maxcost = 1 + Math.floor((actor.classes.bard.system.level - 6) / 3);
} else if (typeof mCl.gestalt !== "undefined") {
	if (mCl.gestalt.name.toLowerCase().includes('bard')) {
		maxcost = 1 + Math.floor((mCl.gestalt.level - 6) / 3);
	}
} else {
	return;
}
...
//	BUFF DATA
const buffs = actor._itemTypes.buff.filter(b => b.system.tag.includes('bladethirst') && b.system.level <= maxcost);
const buff = {
	id: "",
	name: "",
	feature: "",
