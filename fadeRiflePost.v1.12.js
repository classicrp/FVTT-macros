...
// 	----------
	const curVer = 'v1.12';
	const head = `Macro.fadeRiflePost(${curVer}): `;
	const msg = '';
	const failure = false;
...
//	debugger;
	...
	if (isMisfire) {
		...
		if (isBroken) {
			...
			chatMessage(`<p><span style="font-family: Arial">${actor.name}&apos;s <i>${item.name}</i> has <b>exploded</b> causing @Damage[1d10;vigor]{1d10 Vigor}  and @Damage[2;wounds]{2 Wounds}. It is now <b>destroyed</b>.</span></p>`);
			const destroyed = `system.flags.dictionary.destroyed`;
			await item.update({	[destroyed]: 'true' });
			curHP = 0;
			// Play the sound for rifle blowing up
			...
			const fired = `system.flags.dictionary.firedRifle`;
			let shots = 0;
			await item.update({	[fired]: shots });
			curHP = Math.floor(curHP / 2);
			// Play the sound for the revolver misfiring
			const path = "worlds/pf1e/sbcimport/weapons/Rifle/rifle-clip-empty-98832.mp3";
			await foundry.audio.AudioHelper.play({src: path, volume: 0.7});
	...
	await foundry.audio.AudioHelper.play({src: path, volume: 1.0});
}
