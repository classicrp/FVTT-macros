...
//	debugger;
	...
	const isMisfire = shared.attacks[0].chatAttack.attack.isMisfire;
	// old: shared.chatData.system.rolls.attacks[0].ammo.misfire;
	
	if (isMisfire) {

		// lookup item boolean
		const isBroken = item.isBroken; // old: system.broken;
		...
		if (isBroken) {
			...
			await item.setItemDictionaryFlag('destroyed', 'true');
			/* old v1.12: item.update({	[destroyed]: 'true' }); */
			curHP = 0;
			...
			let shots = 0;
			await item.setItemDictionaryFlag('firedRevolver', shots);
			/*  old v1.12: item.update({	[fired]: shots }); */
