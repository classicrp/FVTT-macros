...
*/
	...
	const curVer = 'v1.24';
	const head = `Macro.addEresElectricity2ToActor(${curVer}): `;
	let msg = '';
	let failure = Boolean(false);
//	=========================================================

	const baseAttr = "system.traits.eres";
	const refActor = actor.toObject();
	const eres = refActor.system.traits.eres;
	
/*	eres is an object with these two elements; { 
		custom: a string,
		value: [] an array
	}
*/

//	attribute for custom eres 
	const cAttr = "custom";
//////////////////////////////////////////////////////////////////////////////
//	CHANGES FOR CUSTOM eres 							//
//////////////////////////////////////////////////////////////////////////////
//	this is the string value you change for a custom eres, comma separated for more 	//
//	than one or left blank ("") if not using it.									//
	let cVal = ""																
//////////////////////////////////////////////////////////////////////////////

/* 	For standard entries the must have the following components
		amount: this will be your ERES value.
		...
		list ( as these are hardcoded in "pf1e" ); {
				...
				"Magic", "Negative", "Positive", "Sonic" }
			
*/

//	attribute for the array of known ERES
	const vAttr = "value";
//////////////////////////////////////////////////////////////////////////////
//	CHANGES FOR KNOWN ERES 		        									//
//////////////////////////////////////////////////////////////////////////////
// 	change this number to the amount you want for eres.  set to 0 (zero) if you are 	//
//	not	using a known eres.															//
	const myAmount = 2;																
//////////////////////////////////////////////////////////////////////////////
...
//	check the custom value first 
		...
		*/
			...
			//	add it to the existing string 
				cVal = eres.custom + ", " + cVal;
			}
		}
	// 	we add the value into the string
		if (cVal.length !==0) {
			await foundry.utils.setProperty(eres, cAttr, cVal);
		}

	//	check that we want to add a known value
		if (myObj.amount == 0 && myObj.types.length == 0) {
		//	we don't
			msg = "We are not assigning a known eres."
			console.log(head + msg);
		
		} else {
		//	make sure its not already there
			if (getObjectIndex(eres.value, myObj) == -1) {
			//	not found so add the object eres to the array
				await eres.value.push(myObj);
			}
		}
...
/*  Now the array */
		if (eres.value.length == 0) {
		//	there are no entries in the array, we are done 
		} else {
		//	we want to remove the correct item out of the array 
			let foundAt = getObjectIndex(eres.value, myObj);
			if (foundAt !== -1) {
				delete eres.value[foundAt];
				//	now fix the array
				await repairArray(eres.value);
			}
		};
	}
	//	let everything catch up
	await actor.update({'system.traits.eres': eres});
