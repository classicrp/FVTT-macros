...
//	grab the last chat message
	...
	//  save DC
		...
		const curSav = await parseInt(foundry.utils.getProperty(curBuff, savAttr)); 
		let saves = 0;
...
//				await curBuff.update({ [frmAttr]: (2 * frmDmg) });		
		...
		saves = curSav + saves;
		await curBuff.setItemDictionaryFlag('saves', saves);	//update({ [savAttr]: saves });
		if (failure) {
